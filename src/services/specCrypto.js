// Sealed (encrypted) app specs are read through flux-spec's decrypt lifecycle:
// EncryptedSpec.decrypt(provider) -> DecryptedCanonicalSpec. flux-spec is transport-
// agnostic — it never knows how the bytes are decrypted — so the transport is supplied
// here. A single provider adapter carries the app identity and delegates to a
// version-specific open function; the two are registered on the flux-spec
// EncryptedSpec classes so decrypt() dispatches by version with no branching at the
// call site.
//
// This mirrors the load balancer's implementation deliberately. Both services open the
// same envelopes against the same decrypt service, and a second dialect of the same
 ...
 */
async function unwrapV8Key(http, endpoint, appName, owner, wrappedKey, retries) {
  const payload = {
    fluxID: owner,
    appName,
  ...
  for (let attempt = 0; attempt < retries.attempts; attempt += 1) {
  ...
  throw new Error(`could not unwrap key for ${appName} after ${retries.attempts} attempts`);
}
 ...
 *   retries: { attempts: number, delayMs: number },
 * }} deps
 * @returns {Promise<void>}
 */
async function registerSpecDecryptProviders({ http, endpoints, retries }) {
  ...
  const openV8 = async (encrypted, _aad, appName, owner) => {
    ...
    const aesKey = await unwrapV8Key(http, endpoints.rsaDecrypt, appName, owner, wrappedKey, retries);
